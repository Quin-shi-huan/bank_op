            ...
            data=[
                (
                    [
                        ...
                        mock.call.filter(JuridicPersonTable.id == 1),
                        mock.call.one()
                    ],
                    [
                    JuridicPersonTable(
                        ...
                        idade=30,
                        email_corporativo="joao@email.com",
                        celular="11999999999",
                        categoria="C",
                        faturamento=5000.0,
                        saldo=2000.0
...
def test_insert_person():
    ...
    repo.insert_person(
        faturamento=3000.0,
        idade=28,
        nome_fantasia="Maria Oliveira",
        celular="11988888888",
        email_corporativo="maria@email.com",
        categoria="A",
        saldo=1500.0
    )
...
def test_insert_person_exception():
    ...
    with pytest.raises(Exception, match="Commit failed"):
        repo.insert_person(
            faturamento=4000.0,
            idade=35,
            nome_fantasia="Erro Exemplo",
            celular="0000000000",
            email_corporativo="erro@email.com",
            categoria="erro",
            saldo=100.0
        )
...
def test_get_person():
    ...
    mock_connection.session.filter.assert_called_once_with(JuridicPersonTable.id == 1)
